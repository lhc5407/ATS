...
log_filename = datetime.now().strftime("ats_quantum_log_%Y%m%d_%H%M%S.log")
...
logging.info("ATS_Quantum 프로그램 시작")
...
8. PARAMETER DICTIONARY:
   - Tier Params MUST contain EXACTLY: {"target_atr_multiplier", "stop_loss", "atr_mult", "timeout_candles", "adaptive_breakeven_buffer", "adx_strong_trend_threshold"}.
...
def get_strategy_score(name: str, prev: dict, curr: dict, price: float) -> float:
    try:
        ...
        if name == "bollinger_breakout":
            ...
            # 밴드 상단을 돌파하고 밴드폭(BW)이 확장될 때 고득점
        ...
        if name == "stoch_rsi": 
            ...
            return min(100.0, max(0.0, 60.0 + (diff * 2))) if curr.get('st_rsi_k', 0) > 50 else 0.0
            
        if name == "stochastics": 
            diff = curr.get('stoch_k', 0) - curr.get('stoch_d', 0)
            return min(100.0, max(0.0, 60.0 + (diff * 2))) if curr.get('stoch_k', 0) > 50 else 0.0

        if name == "bollinger_bandwidth":
            diff_pct = ((curr['bb_bw'] - prev['bb_bw']) / max(prev['bb_bw'], 0.0001)) * 100
            return min(100.0, max(0.0, 50.0 + diff_pct * 10)) # 변동성 확대에 더 민감하게 반응
        
        if name == "bollinger": 
            bb_range = curr['bb_u'] - curr['bb_l']
            if bb_range <= 0: return 0.0
            pct_b = ((price - curr['bb_l']) / bb_range) * 100
            # 밴드 상단(100%)에 가까울수록 고득점 (추세 추종)
            return min(100.0, max(0.0, pct_b))

        if name == "volume":
            vol_factor = safe_float(STRAT.get('vol_factor'), 2.0)
            vol_sma = safe_float(curr.get('vol_sma'), 0.0001)
            curr_vol = safe_float(curr.get('volume'), 0.0)
            return min(100.0, (curr_vol / max(vol_sma * vol_factor, 0.0001)) * 100)
            
        if name == "rsi":
            curr_rsi = safe_float(curr.get('rsi'), 50.0)
            if curr_rsi > 85: return 30.0 # 과열 감점
            if curr_rsi >= 60: return min(100.0, 60.0 + (curr_rsi - 60) * 2) # 60~80 구간 고득점
            return max(0.0, curr_rsi - 20) # 낮은 RSI는 저득점
            
        if name == "macd":
            macd_h = safe_float(curr.get('macd_h'), 0.0)
            macd_h_diff = safe_float(curr.get('macd_h_diff'), 0.0)
            # 0선 위에서 히스토그램이 확장될 때 만점
        ...
        if name == "z_score":
            ...
            # +1.5 ~ +2.5 상방 발산 구간일 때 고득점
            if 1.5 <= z <= 2.5: return 100.0
            if z > 2.5: return 60.0 # 너무 높으면 상투 위험으로 약간 감점
            return max(0.0, z * 20)
            
        if name == "sma_crossover":
            # 정배열 확인 (현재가 > 20 > 50)
            price_above_20 = price > curr.get('sma_long', 0)
            ma_20_above_50 = curr.get('sma_long', 0) > curr.get('sma_50', 0)
            if price_above_20 and ma_20_above_50: return 100.0
            if price_above_20: return 60.0
            return 0.0
...
def load_config():
    ...
    path = os.path.join(base_path, "config_quantum.json")
...
async def ai_analyze(ticker, data, mode="BUY", no_trade_hours=0.0, win_rate=50.0, recent_wins=None, mtf_trend="알수없음", buy_price=0.0, market_regime=None, ignore_cooldown=False):
    ...
    if mode == "OPTIMIZE":
        ...
        CRITICAL RULE: You MUST prioritize momentum indicators like 'bollinger_breakout' and 'sma_crossover'. Focus on catching the early stage of a trend.
        ...
        Mission: Rate the trade performance (0-100) based on our Trend Following strategy, and suggest improvements in Korean. Provide JSON.
        ...
        {warning_msg}
        Strategy: Trend Follower & Breakout Trader (추세 추종 및 돌파 매매). 
        Mission: You are the FINAL GATEKEEPER. Evaluate risk/reward and provide JSON. Look for strong breakouts with confirmed volume.
        CRITICAL: Calculate your OWN independent AI Confidence Score (0-100). Do NOT simply copy the Python Score! Weigh the risks against the upside potential. Minor flaws (e.g., slight overbought, imperfect orderbook) are acceptable if the momentum is strong enough. Output "BUY" if the setup offers a favorable risk/reward ratio (AI Score >= 70). Output "SKIP" only if it's a clear fakeout or weak momentum.
        """
        ...
        Indicators: {clean_data}
        {warning_msg}
        Situation: Python preemptive purchase based on 'Trend Follower & Breakout Trader' (추세 추종 및 돌파 매매).
        Mission: Re-verify entry quality. Focus on confirmed breakouts, strong volume, and continuing momentum. 
        CRITICAL: Calculate your OWN independent AI Confidence Score (0-100). Do not blindly reject if it's a strong trend. If it's a clear fakeout with NO momentum, decision is SKIP (emergency exit). Otherwise, decision is BUY. Provide JSON.
        """
    else: return None
        ...
        try:
            ...
            if mode in ("POST_BUY_REPORT", "BUY"):
                ...
                exit_plan = {
                    ...
                    "stop_loss": max(-2.0, min(-0.5, float(raw_plan.get('stop_loss', -1.5)))), # 퀀텀: 손절선을 -1.5% ~ -2.0%로 타이트하게 조정
                    "atr_mult": max(0.5, min(4.0, float(raw_plan.get('atr_mult', 1.0)))), # 퀀텀: ATR 멀티플을 1.0으로 조정
                    "timeout": max(2, min(15, int(raw_plan.get('timeout', 5)))) # 퀀텀: 타임아웃 캔들을 짧게 조정
                }
...
async def ai_self_optimize(trigger="manual"):
    ...
    if res and 'strategy' in res:
        ...
        if 'range_active_logic' in new_data: 
            # 퀀텀: 횡보장 지표는 추세 전환 초입 감지에 활용 (RSI, 스토캐스틱 강화)
        ...
        for k, v in new_data.items():
            if k in STRAT and k not in ignore_list:
                try:
                    ...
                    elif k == 'fgi_v_curve_bottom': v = max(20.0, min(50.0, float(v))) # 퀀텀: FGI 'Greed' 구간 조정
                    elif k == 'fgi_v_curve_max': v = max(1.5, min(3.0, float(v))) # 퀀텀: FGI Greed 최대 가중치
                    elif k == 'fgi_v_curve_min': v = max(0.5, min(1.0, float(v))) # 퀀텀: FGI Greed 최소 가중치
                    elif k == 'fgi_v_curve_greed_max': v = max(1.0, min(2.5, float(v))) # 퀀텀: 기존 Greed Max는 삭제 (v_max, v_min으로 대체)
                    elif k == 'deep_scan_interval': v = max(900, min(1800, int(v)))
                    elif k == 'pass_score_threshold': v = max(75, min(90, int(v))) # 퀀텀: 통과 점수 상향
                    elif k == 'guard_score_threshold': v = max(60, min(75, int(v))) # 퀀텀: 방어 점수 상향
                    elif k == 'sell_score_threshold': v = max(40, min(55, int(v))) # 퀀텀: 매도 점수 상향
                    elif k == 'rsi_low_threshold': v = max(40.0, min(60.0, float(v))) # 퀀텀: RSI 낮은 임계값 상향
                    elif k == 'rsi_high_threshold': v = max(70.0, min(85.0, float(v))) # 퀀텀: RSI 높은 임계값 상향
        ...
        if changes: await send_msg(f"✨ <b>퀀텀 전략 진화 완료 </b>\n\n" + "\n".join(changes) + f"\n\n💡 <b>AI:</b>\n{ai_reason}")
...
async def run_full_scan(is_deep_scan=False):
    ...
    # 🟢 API 호출 제한 분산 (Jitter): 두 봇이 동시에 API를 찌르는 것을 방지하기 위해 랜덤 지연
    import random
    ...
    # 퀀텀: 비트코인이 단기 하락 중이거나 리스키하면 관망 (하락장 매수 금지)
    if btc_short['is_risky'] or btc_short['trend'] == "단기 하락":
        new_status = "💤 관망 (BTC 하락/변동성 위험)"
        SYSTEM_STATUS = new_status
        if is_deep_scan: 
            await send_msg("💤 <b>[추세 추종 안전 장치]</b> 비트코인 하락 추세에서는 매수를 중단하고 철저히 현금을 관망합니다.")
            last_global_buy_time = time.time()
            return
    else:
        SYSTEM_STATUS = new_status
    ...
    if is_deep_scan:
        if consecutive_empty_scans >= 3:
            await send_msg(f"⏳ <b>[{consecutive_empty_scans + 1}회 연속 관망]</b> 돌파 타점이 포착되지 않습니다. 안전하게 대기합니다.")
        else: 
            await send_msg(f"⏰ <b>{consecutive_empty_scans + 1}번째</b> 퀀텀 정규 스캔 중...")
    ...
    b_ath = score_mods.get('bonus_all_time_high', 20)
    b_vol_exp = score_mods.get('bonus_volume_explosion', 30)
    p_btc_weak = score_mods.get('penalty_btc_weakness', -15)
    ...
    for item in indicator_results:
        ...
        if fgi_value >= 80: # Extreme Greed
            dynamic_fgi_mult = v_min # 상투 위험으로 가중치 축소
        elif fgi_value >= 50: # Greed
            dynamic_fgi_mult = v_min + (((fgi_value - 50) / 30) * (v_max - v_min))
        else: # Fear
            dynamic_fgi_mult = v_min # 추세 추종에 부적합한 공포장
            
        dynamic_fgi_mult = round(dynamic_fgi_mult, 2)
            
        try:
            normalized_greed = (dynamic_fgi_mult - v_min) / (v_max - v_min)
        except ZeroDivisionError:
            normalized_greed = 0.5
        normalized_greed = max(0.0, min(1.0, normalized_greed)) 
        
        rsi_mult = 1.0 + (1.0 * normalized_greed)        
        boll_mult = 1.5 + (0.5 * normalized_greed)       
        st_mult = 1.0 + (1.0 * normalized_greed)         
        vol_mult = 1.0 + (1.5 * normalized_greed)        
        bb_break_mult = 1.5 + (1.5 * normalized_greed)   
        ...
        # 🟢 [Pylance 및 런타임 방어] 퀀텀 보너스 점수 로직 안전성 강화
        # bb_u가 없으면 0이 아니라 무한대(inf)로 두어 실수로 돌파했다고 착각하는 것을 방지
        bb_u_val = safe_float(curr.get('bb_u'), float('inf')) 
        is_breakout = (curr_close >= bb_u_val)
        is_vol_explosion = curr_volume > (vol_sma_val * 3.0) # 평소 거래량 3배 이상
        
        if is_breakout and is_vol_explosion:
            pass_score += b_vol_exp
            if is_deep_scan: curr['warning'] = curr.get('warning', '') + f"\n🚀 [거래량 동반 돌파] 강력한 수급과 함께 저항 돌파! (+{b_vol_exp}점)"
            
        # 🟢 [Pylance 방어] 신고가 갱신 확인 시 데이터프레임 유효성(empty) 및 safe_float 검사 추가
        df_high_check = OHLCV_CACHE.get(t)
        if isinstance(df_high_check, pd.DataFrame) and not df_high_check.empty:
            ath_price = safe_float(df_high_check['high'].max())
            # ath_price가 0보다 클 때만 정상적인 돌파로 인정
            if curr_close >= ath_price and ath_price > 0:
                pass_score += b_ath
                if is_deep_scan: curr['warning'] = curr.get('warning', '') + f"\n🏆 [신고가 돌파] 최근 매물대를 돌파한 신고가 영역 진입! (+{b_ath}점)"

        if btc_short['trend'] == "단기 하락":
            pass_score += p_btc_weak # 퀀텀은 비트 하락 시 페널티
            
        # 🟢 치명적 결함 검사 (퀀텀 리버스)
        fatal_flaw = False

        # (1) 슈퍼트렌드 하드락: 하락 추세(-1)일 경우 무조건 탈락
        if curr.get('ST_DIR', 1) == -1:
            fatal_flaw = True
            if is_deep_scan: curr['warning'] = curr.get('warning', '') + "\n🚫 [추세 추종 하드락] 슈퍼트렌드가 하락 추세(-1)입니다."

        # (2) 역배열 진입 금지: 가격이 EMA 10 또는 SMA 50 아래에 있으면 탈락
        elif curr_close < curr.get('ema_10', 0) or curr_close < curr.get('sma_50', 0):
            fatal_flaw = True
            if is_deep_scan: curr['warning'] = curr.get('warning', '') + "\n🚫 [역배열 진입 금지] 가격이 주요 이동평균선(EMA 10, SMA 50) 아래에 있습니다."

        # (3) 가짜 돌파(Fake Breakout) 필터: 저항 돌파 시 거래량이 평소 2배 미만이면 탈락
        elif is_breakout and curr_volume < (vol_sma_val * 2.0):
            fatal_flaw = True
            if is_deep_scan: curr['warning'] = curr.get('warning', '') + f"\n🚫 [가짜 돌파 필터] 저항 돌파 시 거래량({curr_volume/vol_sma_val:.1f}배)이 부족합니다 (최소 2배 필요)."
        ...
        if fatal_flaw: 
            pass_score = -999
            if is_deep_scan: curr["warning"] = curr.get("warning", "") + "\n🚫 [퀀텀 프리패스 차단] 치명적 결함 발견!"
        
        if pass_score < 0: 
            continue

        min_score_threshold = STRAT.get("min_score", 70) # 퀀텀은 기본 점수 70점 이상 요구

        # 상황별 점수 임계값 완화 (퀀텀)
        if curr.get('ST_DIR', -1) == 1: min_score_threshold = min(min_score_threshold, 65)
        if curr_close > curr.get('ema_10', 0) and curr_close > curr.get('sma_50', 0): min_score_threshold = min(min_score_threshold, 60)
        if curr.get('rsi', 50) >= 60 and curr.get('rsi', 50) < 80: min_score_threshold = min(min_score_threshold, 60)
        if z_score_val >= 1.5 and z_score_val <= 2.5: min_score_threshold = min(min_score_threshold, 55)
        if is_vol_explosion: min_score_threshold = min(min_score_threshold, 60)
        ...
        # 퀀텀은 비트코인 단기 하락 시 매수 자체를 막음
        if btc_short['trend'] == "단기 하락": continue
        
        if check_correlation_risk(t, list(held_dict.keys())): continue 
        
        # 퀀텀은 과매수에서 진입하지 않으므로 RSI 상단 필터 제거
    ...
    while True:
        try:
            ...
            for update in updates:
                ...
                if cmd == "보고": await send_msg(await build_report("퀀텀 실시간 보고", is_running))
                    ...
                    asyncio.create_task(generate_daily_proposal(bot_name="Quantum"))
                    ...
                    trade_data.clear()
                    TRADE_DATA_DIRTY = True
                    await send_msg(f"🚨 <b>퀀텀 수동 전량 매도 완료 ({sold_count}개 종목 청산)</b>")
...
async def main():
    ...
    try:
        ...
        instance_lock.bind(('127.0.0.1', 65433)) # 퀀텀 봇은 65433 포트를 사용하여 클래식과 분리합니다.
    except socket.error:
        logging.error("❌ 이미 ATS_Quantum 봇이 실행 중입니다. 중복 실행을 방지하기 위해 즉시 종료합니다.")
        return # 이미 포트가 잠겨있다면 조용히 프로그램을 종료!
    
    # 🟢 API 호출 제한 분산 (Jitter): 클래식 봇과 동시에 시작되는 것을 방지하기 위해 1~10초 사이 랜덤 대기
    ...
    await send_msg(f"⚔️ <b>ATS_Quantum 가동 시작 </b>\n\n{COMMAND_HELP_MSG}")
    ...
    while True:
        try:
            ...
            for ticker in monitoring_tickers:
                ...
                if ticker in held:
                    ...
                    if ticker in INDICATOR_CACHE:
                        ...
                        # 퀀텀: 탐욕(Greed) 구간에서 가중치 부여, 극단적 탐욕은 경계
                        if fgi_value >= 80: # Extreme Greed
                            dynamic_fgi_mult = v_min # 상투 위험으로 가중치 축소
                        elif fgi_value >= 50: # Greed
                            dynamic_fgi_mult = v_min + (((fgi_value - 50) / 30) * (v_max - v_min))
                    ...
                    # 퀀텀: 이익이 커질수록 트레일링 스탑을 더 타이트하게 조여 수익을 보존
                    dynamic_mult = current_atr_mult
                    if max_p_rate >= 5.0: dynamic_mult = current_atr_mult * 0.3  # 강력한 추세에서는 바짝 붙음
                    ...
                    # 샹들리에 스탑 강화: 고점 대비 -2% 이탈 시 즉시 청산 (수익 보존)
                    ...
                    # 퀀텀: 본절 방어막 강화 (+1.0% 이상 도달 시 작동)
                    if t.get('breakeven_locked') or (t.get('guard') and max_p_rate > 0.6): 
                        hard_breakeven_floor = avg_p * 1.005 # 최소 0.5% 수익 확보
                    ...
                    sell_conditions = [
                        # 퀀텀: 추세가 강력하면 익절을 미루고 트레일링 스탑에 맡김 (Let winners run)
            ...
            if now_ts - last_loss_check_time >= 14400:
                ...
                try:
                    ...
                    if wr < 70.0 and wr < last_checked_win_rate:
                        await ai_self_optimize(trigger="auto") # 퀀텀 전략 자동 최적화
            ...
            logging.error(f"❗ 퀀텀 메인 루프 예외 발생: {e}\n{traceback.format_exc()}")
